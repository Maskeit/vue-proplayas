                    ...
                    <div>
                        <div class="flex items-center justify-center w-full">
                            <label for="dropzone-file" class="relative flex flex-col items-center justify-center w-full h-64 
                                ...
                                :class="form.profile_picture_node_file ? 'border-0 bg-white dark:bg-gray-800' : ''"
                                @dragover.prevent @drop.prevent="handleDrop">
                                <!-- Si NO hay imagen, muestra dropzone con instrucciones -->
                                <div v-if="!form.profile_picture_node_file"
                                ...
                                <div v-else class="relative flex flex-col items-center justify-center p-4">
                                    <img :src="form.profile_picture_node_file" alt="Foto de perfil"
...
interface FormData {
    profile_picture_node_file: string | ArrayBuffer | null;
}
...
const form = ref<FormData>({...props.nodeData});
...
const handleDrop = (event: DragEvent) => {
  ...
  if (file) {
    ...
    reader.onload = () => {
      form.value.profile_picture_node_file = reader.result as string;
    };
...
const removeImage = () => {
    form.value.profile_picture_node_file = null;
};
const submitForm = () => {
    emit("uploadImg", form.value);
    closeModal();
};
</script>